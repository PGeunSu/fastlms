...
public class PagingUtil {
  ...
  public PagingUtil(long totalCount,long pageSize,long pageIndex ,String queryString) {
  ...
  public PagingUtil(long totalCount, long pageIndex, String queryString, long pageSize, long pageBlockSize) {
    this.totalCount = totalCount;
    this.pageIndex = pageIndex;
    this.queryString = queryString;
    this.pageSize = pageSize;
  ...
  private void init(){
    ...
    totalBlockCount = totalCount / pageSize + (totalBlockCount % pageSize > 0 ? 1 : 0);

    startPage = ((pageIndex - 1) / pageBlockSize) * pageBlockSize + 1;

    endPage = startPage + pageBlockSize - 1;

    if (endPage > totalBlockCount) endPage = totalBlockCount;
